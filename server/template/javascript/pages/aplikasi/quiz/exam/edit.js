...
class PageAplikasiQuizExamEdit extends React.Component{
  ...
  render() {     
    const {tingkatan,mapel,semester,uploadProgress,uploadDisable,akses,userID,userData,nama,mulai,kisi,selesai,nilai,paketSoal,paketData,loading,errorNama,errorUserID,errorNilai,errorMulai,errorSelesai,errorPaketSoal} = this.state; 
    ...
    <div className="konten"> 
        ...
        <div className="headings">    
          ...
          <Breadcrumb homeUrl="/aplikasi" homeText="Aplikasi"> 
            ...
            <li><Link to="#"><span>Mengubah ujian</span></Link></li>   
          </Breadcrumb>   
        </div>
  ...
  fetchData = (tingkat,mapel,semester,exam) => {   
    axios.get(
      ...
      this.setState({
        ...
        userID:response.data.message.data.user_id,
  ...
  /*--- post new ujian ----*/
  updateExam = async () => {
    const {croppedImageUrl,id,userID,nama,nilai,mulai,selesai,paketSoal} = this.state;    
    ...
    axios({
      ...
      headers: {
        ...
        {           
          toast.success("Data ujian "+nama+" berhasil diperbarui");                             
          this.navigate(-1); 
        }
    }).catch(error => {                   
      if(error.response.status == 400){                       
