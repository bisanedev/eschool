...
class PageAplikasiQuizEssayMapel extends React.Component{
  ...
  render() {
    ...
    <div className="konten"> 
        ...
        {data.length > 0 && !isLoading && data.map((value,k) => (                   
          <MenuText key={k} url={"/aplikasi/quiz/essay/"+ this.tingkatID +"/"+value.id} title={value.nama} jumlah={"Jumlah soal "+value.jumlah} style={{backgroundColor:value.color}} color="#fff"/>        
        ))}
  ...
  fetchData = (id) => {     
    ...
    axios.get(
      window.location.origin + `/api/pendidik/aplikasi/quiz/index/essay/${id}?&nocache=`+Date.now()
    ).then(response => {      
      this.setState({
...
export default PageAplikasiQuizEssayMapel;