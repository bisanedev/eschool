...
class PageAplikasiQuizPaketSoal extends React.Component{
      ...
      <Helmet>
        ...
        <Table>
          ...
          <Table.Body>
            ...
            {data.length === 0 && !isLoading && <Table.Empty nama="paket soal" /> }              
          </Table.Body>
          <Table.Footer>
            <div className="w-50 ph2">
  ...
  fetchData = (tingkatID,mapelID,semesterID) => { 
    ...
    axios.get(
      window.location.origin + `/api/pendidik/aplikasi/quiz/paket/${tingkatID}/${mapelID}/${semesterID}?`+ `${total ? 'total=' + total : ''}` + `${page ? '&page=' + page : ''}`+ `${cari ? '&cari=' + cari : ''}` +"&nocache="+Date.now()
    ).then(response => {      
      this.setState({
...
export default PageAplikasiQuizPaketSoal;