...
class PageAplikasiQuizExamSemester extends React.Component{
  ...
  render() {
    ...
    <div className="konten"> 
        ...
        {data.length > 0 && !isLoading && tingkatan != null && mapel != null && data.map((value,k) => (                  
          <MenuText key={k} url={"/aplikasi/quiz/exam/"+ this.tingkatID+"/"+this.mapelID +"/"+value.id} title={"Semester "+value.semester} jumlah={"Jumlah ujian "+value.jumlah} subtitle={value.tahun} color="#333"/>        
        ))}
  ...
  fetchData = (tingkat,mapel) => {     
    ...
    axios.get(
      window.location.origin + `/api/pendidik/aplikasi/quiz/index/exam/${tingkat}/${mapel}?&nocache=`+Date.now()
    ).then(response => {      
      this.setState({
...
export default PageAplikasiQuizExamSemester;