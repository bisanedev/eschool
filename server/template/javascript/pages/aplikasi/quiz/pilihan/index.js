...
class PageAplikasiQuizPilihan extends React.Component{
    ...
    <div className="konten"> 
        ...
        <div className="headings">
          ...
          <Breadcrumb homeUrl="/aplikasi" homeText="Aplikasi">                                            
            ...
            <li><a href="#"><span>Tingkatan kelas</span></a></li>         
          </Breadcrumb>
        </div>        
        <div className="mw9 center">
        <div className="cf ph3 mb3 flex flex-wrap">
        {data.length > 0 && !isLoading && data.map((value,k) => (                   
          <MenuText key={k} url={"/aplikasi/quiz/pilihan/"+value.id} title={value.nama} color="#333"/>        
        ))}
  ...
  fetchData = () => {     
    ...
    axios.get(
      window.location.origin + "/api/pendidik/aplikasi/quiz/tingkatan?&nocache="+Date.now()
    ).then(response => {      
      this.setState({
